...
def clean_content(html_content):
    """ Remove unwanted text and HTML tags. """
    ...
    # Parse HTML content to remove unwanted tags (like <font>, <img>, etc.)
    soup = BeautifulSoup(html_content, "html.parser")
    
    # Remove any unwanted tags (like <br>, which we don't want to appear in Telegra.ph)
    for tag in soup.find_all(["br", "font"]):
        tag.decompose()

    return str(soup)  # Return as raw HTML content
...
def create_telegraph_post(title, content):
    ...
    response = requests.post(TELEGRAPH_CREATE_URL, json={
        ...
        "content": [{"tag": "html", "children": [content]}],
    })
...
def check_feed():
    ...
    # Process RSS entries in reverse order to maintain correct post sequence
